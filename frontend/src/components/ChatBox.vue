<!--    
  ...
  const handleLocationClick = (msg) => {
    if (msg.fromMe) {
      // 如果是自己发的消息，直接跳转到自己的位置
      const url = `https://www.amap.com/?ll=${msg.latitude},${msg.longitude}&zoom=15`
      window.open(url, '_blank') // 打开自己的位置
    } else {
      // 如果是对方发的消息，生成从对方位置到我的位置的导航路线
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          const userLatitude = position.coords.latitude
          const userLongitude = position.coords.longitude
  
          const url = `https://www.amap.com/?s=s%26query=${userLatitude},${userLongitude}&d=${msg.latitude},${msg.longitude}`
          window.open(url, '_blank') // 打开高德地图并生成导航
        })
      } else {
        alert('定位功能不可用')
      }
  ...
  if (isLocationOwner) {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        const url = `https://uri.amap.com/navigation?from=${userLongitude},${userLatitude},我的位置&to=${msg.longitude},${msg.latitude},对方位置&mode=car`
